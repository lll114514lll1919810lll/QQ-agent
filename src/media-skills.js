// 媒体技能（纯 Node.js，无 Python 子进程）
// - bilibili：B 站公开接口 + 可选登录态（cookie 存 data/media/）
// - netease_music：NeteaseCloudMusicApi HTTP（默认公开接口，不发 cookie）
//
// 安全契约：
// 1. cookie 只从 data/media/*.json 读，不进 argv、不进工具描述、不进工具结果
// 2. 输出回传前统一抹除 cookie 片段与常见 token 形态
// 3. 命令白名单 + 参数校验
// 4. 同会话限频
...
import { unquoteJsonString } from './util.js';

const MEDIA_DIR = path.join(DATA_DIR, 'media');
const BILI_COOKIE_FILE = path.join(MEDIA_DIR, 'bilibili-cookies.json');
const NCM_COOKIE_FILE = path.join(MEDIA_DIR, 'netease-cookies.json');
...
function writeCookieFile(file, cookies) {
  ensureMediaDir();
...
function secretStrings() {
  const out = [];
  for (const cookies of [readCookieFile(BILI_COOKIE_FILE), readCookieFile(NCM_COOKIE_FILE)]) {
...
const TOKEN_KEYS = ['SESSDATA', 'bili_jct', 'DedeUserID__ckMd5', 'DedeUserID', 'MUSIC_U', 'NMTID', '__csrf', 'buvid3', 'buvid4', 'SESSDATA_Expires'];

function redact(text) {
  let out = String(text ?? '');
  for (const secret of secretStrings()) {
    if (out.includes(secret)) out = out.split(secret).join('[已隐藏]');
  }
  const re = new RegExp('\\b(' + TOKEN_KEYS.join('|') + ')=[^;,\\s\'"\\]}]+', 'gi');
  out = out.replace(re, (_m, key) => key + '=[已隐藏]');
  return out;
}

function clampInt(value, min, max, fallback) {
  const n = Math.floor(Number(unquoteJsonString(value)));
  if (!Number.isFinite(n)) return fallback;
  return Math.min(max, Math.max(min, n));
}

function pickEnum(value, allowed, fallback) {
  const s = String(unquoteJsonString(value) ?? '').trim();
  return allowed.includes(s) ? s : fallback;
}

function cleanText(value, opts) {
  const max = (opts && opts.max) || 60;
  const name = (opts && opts.name) || 'keyword';
  const raw = String(unquoteJsonString(value) ?? '');
  let s = '';
  for (const ch of raw) {
  ...
  s = s.trim();
  if (!s) throw new Error('缺少 ' + name);
  return s.slice(0, max);
}

function cleanBvid(value) {
  const s = String(unquoteJsonString(value) ?? '').trim();
  const m = /^(BV[0-9A-Za-z]{10})$/.exec(s);
  if (m) return m[1];
  const fromUrl = /(BV[0-9A-Za-z]{10})/.exec(s);
  if (fromUrl) return fromUrl[1];
  throw new Error('bvid 不合法（应形如 BV1xx411c7mD，可直接给完整链接）：' + s.slice(0, 40));
}

function requireId(value, label) {
  const name = label || 'id';
  const n = Math.floor(Number(unquoteJsonString(value)));
  if (!Number.isFinite(n) || n <= 0) throw new Error('需要有效的数字 ' + name + '（从 search/song 等结果里取，不要自己编）');
  return String(n);
}
...
function wan(n) {
  return Math.floor((Number(n) || 0) / 10000);
}

function truncate(text, max) {
  const limit = Math.max(300, Number(max) || 3500);
...
function checkRate(chatKey, skillKey) {
  const cfg = getConfig().media?.rateLimit ?? {};
  const perMinute = Math.max(1, Number(cfg.perChatPerMinute) || 6);
  const perHour = Math.max(perMinute, Number(cfg.perChatPerHour) || 30);
  const key = chatKey + '|' + skillKey;
  const now = Date.now();
  const times = (callLog.get(key) || []).filter((t) => now - t < 3600000);
  const inMinute = times.filter((t) => now - t < 60000).length;
  if (inMinute >= perMinute) return '本会话调用太频繁（每分钟最多 ' + perMinute + ' 次），过一会儿再试';
  if (times.length >= perHour) return '本会话调用太频繁（每小时最多 ' + perHour + ' 次），稍后再试';
  times.push(now);
  callLog.set(key, times);
  if (callLog.size > 500) callLog.clear();
  return null;
}
